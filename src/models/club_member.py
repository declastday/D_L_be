...
class ClubMember(Base):
    ...
    # ★ 복합 인덱스 추가
    # 활동중인 동아리 조회: user_id + status='active'
    # 동아리 회원 명단 조회: club_id + status='active'
    __table_args__ = (
        Index("ix_club_members_user_id_status", "user_id", "status"),
        Index("ix_club_members_club_id_status", "club_id", "status"),
    )
